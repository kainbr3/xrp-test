...
import (
	...
	t "crypto-braza-tokens-api/clients/ripple/utils/binary-codec/types"
)
...
func EncodeForSigningClaim(json map[string]any) (string, error) {
	...
	channel, err := t.NewHash256().FromJson(json["Channel"])
...
func removeNonSigningFields(json map[string]any) map[string]any {
	...
	for k := range json {
		fi, _ := d.Get().GetFieldInstanceByFieldName(k)
