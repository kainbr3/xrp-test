...
func (u *UInt16) FromJson(value any) ([]byte, error) {
	...
	if _, ok := value.(string); ok {
		tc, err := d.Get().GetTransactionTypeCodeByTransactionTypeName(value.(string))
		if err != nil {
			tc, err = d.Get().GetLedgerEntryTypeCodeByLedgerEntryTypeName(value.(string))
			if err != nil {
				return nil, err
			}
		}
...
func (u *UInt16) ToJson(p *s.BinaryParser, opts ...int) (any, error) {
	b, err := p.ReadBytes(2)
	if err != nil {
		return nil, err
	}
