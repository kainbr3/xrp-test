...
import (
	...
	s "crypto-braza-tokens-api/clients/ripple/utils/binary-codec/serdes"

	d "crypto-braza-tokens-api/clients/ripple/utils/binary-codec/definitions"
)
...
func (t *STObject) ToJson(p *s.BinaryParser, opts ...int) (any, error) {
...
func createFieldInstanceMapFromJson(json map[string]any) (map[d.FieldInstance]any, error) {
	m := make(map[d.FieldInstance]any, len(json))

	for k, v := range json {
		fi, err := d.Get().GetFieldInstanceByFieldName(k)
...
//
//lint:ignore U1000 // ignore this for now
func getSortedKeys(m map[d.FieldInstance]any) []d.FieldInstance {
	keys := make([]d.FieldInstance, 0, len(m))
...
func enumToStr(fieldType string, value any) (any, error) {
	...
	case "TransactionType":
		return d.Get().GetTransactionTypeNameByTransactionTypeCode(int32(value.(int)))
	case "TransactionResult":
		return d.Get().GetTransactionResultNameByTransactionResultTypeCode(int32(value.(int)))
	case "LedgerEntryType":
		return d.Get().GetLedgerEntryTypeNameByLedgerEntryTypeCode(int32(value.(int)))
	default:
		return value, nil
	}
}
